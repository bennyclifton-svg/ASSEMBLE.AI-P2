 ...
 * /api/admin/models
 *
 * GET   — list current model settings for all 5 feature groups
...
export async function PATCH(req: Request) {
    ...
    // Anthropic model IDs are versioned strings — require catalog membership so typos
    // don't get committed. OpenRouter hosts hundreds of models; allow any ID so new
    // releases can be used immediately without a code change + redeploy.
    ...
    if (!before) {
        return NextResponse.json({ error: 'Feature group row missing — was Phase 1 migration run?' }, { status: 500 });
    }

    if (before.provider === provider && before.modelId === modelId) {
    ...
    await recordAdminAction({
        ...
        before: { provider: before.provider, modelId: before.modelId },
