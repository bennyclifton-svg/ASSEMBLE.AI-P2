...
interface RouteParams {
    params: Promise<{ projectId: string; rfiId: string; linkId: string }>;
}
...
function serviceErrorResponse(error: unknown): NextResponse {
    ...
    console.error('[rfis/:id/evidence/:linkId] API error:', error);
    return NextResponse.json({ error: 'Failed to process RFI evidence request' }, { status: 500 });
}
...
export async function DELETE(_request: NextRequest, { params }: RouteParams) {
    ...
    const { projectId, rfiId, linkId } = await params;

    try {
        const rfi = await rfiService.removeEvidence({
            id: rfiId,
            projectId,
