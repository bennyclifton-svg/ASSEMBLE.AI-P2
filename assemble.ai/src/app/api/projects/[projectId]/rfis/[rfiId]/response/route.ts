...
function serviceErrorResponse(error: unknown): NextResponse {
    ...
    console.error('[rfis/:id/response] API error:', error);
    return NextResponse.json({ error: 'Failed to record RFI response' }, { status: 500 });
}
...
export async function POST(request: NextRequest, { params }: RouteParams) {
    ...
    try {
        ...
        const rfi = await rfiService.recordResponse({
            id: rfiId,
            projectId,
