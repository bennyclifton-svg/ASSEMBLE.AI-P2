...
function serviceErrorResponse(error: unknown): NextResponse {
    ...
    console.error('[rfis/promote-note] API error:', error);
    return NextResponse.json({ error: 'Failed to promote RFI note' }, { status: 500 });
}
...
export async function POST(request: NextRequest, { params }: RouteParams) {
    ...
    try {
        ...
        const rfi = await rfiService.promoteFromNote({
            projectId,
