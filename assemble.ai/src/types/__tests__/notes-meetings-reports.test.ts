...
describe('note color helpers', () => {
    it('falls back to the default color for invalid persisted values', () => {
        ...
        expect(getNoteColor('yellow')).toBe(DEFAULT_NOTE_COLOR);
        expect(getNoteColorStyles('yellow')).toBe(NOTE_COLOR_MAP[DEFAULT_NOTE_COLOR]);
    });

    it('returns configured styles for valid colors', () => {
        expect(getNoteColor('purple')).toBe('purple');
        expect(getNoteColorStyles('purple')).toBe(NOTE_COLOR_MAP.purple);
