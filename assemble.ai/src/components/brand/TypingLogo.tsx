...
export function TypingLogo({ className }: TypingLogoProps) {
    ...
    const tick = () => {
        ...
        if (!isDeleting && updatedText === fullWord) {
            ...
            // Triple the rest time for 'SiteWise' (primary brand)
            newDelta = fullWord === 'SiteWise' ? PAUSE_DURATION * 3 : PAUSE_DURATION;
            setIsDeleting(true);
    ...
    return (
        <span className={cn("font-bold italic relative inline-block", className)}>
            ...
            <span className="opacity-0" aria-hidden="true">SiteWise|</span>
