  ...
  if (isLoading) {
    return (
      <div className="bg-[#252526] rounded-lg p-6 border border-[#3e3e42]">
        <h3 className="text-lg font-semibold text-[#cccccc] mb-4">Contractor List</h3>
  ...
  return (
    <div className="bg-[#252526] rounded-lg p-6 border border-[#3e3e42]">
      ...
      <div className="space-y-2 max-h-[500px] overflow-y-auto pr-2">
        {trades.map((trade) => (
          <div key={trade.id} className="flex items-center justify-between px-4 py-3 bg-[#1e1e1e] rounded border border-[#3e3e42]">
            <div className="flex items-center gap-4">
              <Switch
                checked={trade.isEnabled}
                onCheckedChange={(checked: boolean) => toggleTrade(trade.id, checked)}
              />
              <span className={cn("text-[#cccccc]", !trade.isEnabled && "text-[#858585]")}>
                {trade.tradeName}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              {statusTypes.map(({ type, label, icon: Icon, color }) => {
                const status = trade.statuses.find(s => s.statusType === type);
                const isActive = status?.isActive ?? false;

                return (
                  <button
                    key={type}
                    onClick={() => trade.isEnabled && updateStatus(trade.id, type, !isActive)}
                    disabled={!trade.isEnabled}
                    className={cn(
                      "p-2 rounded-full transition-all duration-200",
                      isActive ? "bg-accent shadow-sm" : "opacity-30 hover:opacity-50",
                      !trade.isEnabled && "opacity-10 cursor-not-allowed"
                    )}
