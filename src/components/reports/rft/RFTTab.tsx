...
export function RFTTab({
  ...
  // Handle reset - cancels stalled generation and allows restart
  const handleReset = async () => {
    if (!reportId) return;
    setIsResetting(true);
    try {
      const res = await fetch(`/api/reports/${reportId}/reset`, { method: 'POST' });
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.message || err.error || 'Failed to reset');
      }
      // Disconnect any active stream
      stream.disconnect?.();
      // Refresh data to show updated state
      await mutate();
      toast.success('Report generation reset. You can now regenerate.');
    } catch (err) {
      console.error('Reset failed:', err);
      toast.error(err instanceof Error ? err.message : 'Failed to reset report');
    } finally {
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <button
                className="inline-flex items-center gap-1 px-2 py-0.5 text-xs text-[#cccccc] bg-[#5a1d1d] border border-[#7a3030] rounded hover:bg-[#7a3030] disabled:opacity-50 transition-colors"
                onClick={handleReset}
                disabled={isResetting}
                title="Cancel generation and reset to allow restart"
              >
                {isResetting ? (
            ...
            <span className="text-xs text-[#858585]">
              {completedSections}/{totalSections} sections ({Math.min(progress, 100)}%)
            </span>
          </div>
