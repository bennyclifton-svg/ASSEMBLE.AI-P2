...
async function generateAllSections(
    ...
    // Build initial state
    // Note: discipline name is not passed directly - planningContext contains discipline info
    let state: ReportStateType = {
        projectId,
        reportType: 'tender_request',
        title: '', // Will be populated from report
        discipline: null, // Discipline info available in planningContext
        trade: null,
        documentSetIds,
        reportId,
    ...
    try {
        ...
        await captureReportMemory({
            ...
            reportType: 'tender_request',
            discipline: null, // Discipline name not available here (only ID)
            tableOfContents: toc,
        });
...
export async function POST(request: NextRequest, { params }: RouteParams) {
    try {
        ...
        generateAllSections(
            id,
            report.projectId,
            report.disciplineId ?? null, // Use disciplineId directly for transmittal lookup
            updatedToc,
